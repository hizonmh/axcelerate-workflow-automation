...
| Ezidebit PDF | `parse_ezidebit_pdf()` | Processed Payments Report PDF. Uses line-based regex over extracted text (not table detection) to handle varying column layouts. Only settled "Paid" rows are imported (regex requires both trans and settlement dates). Student ID extracted from Client Contract Ref (MAC ID or numeric ID). Status set to "OK to Upload". Location extracted from header (e.g. "Macallan College - Brisbane") |
